    ...
    void Update() {
        if (MoveToTarget != null) {
            ...
            if (percentComplete > 1.0f) {
                MoveToTarget.DealDamage((int)(baseDamage * Random.Range(damageModifier, 1.0f))); // Deal the appropriate damage to the target on impact (ideally this damage should be handled in the Entity attack method
                Destroy(gameObject); // Remove the projectile. Apply earth-shattering kaboom here?
            }
            else {
            ...
            Debug.LogError("Projectile must have a target specified on creation. Destorying this instance.");
            Destroy(gameObject);
        }
    }
